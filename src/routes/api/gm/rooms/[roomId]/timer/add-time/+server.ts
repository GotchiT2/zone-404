// API endpoint pour ajouter du temps au timer (GM uniquement)
...
export const POST: RequestHandler = async ({ params, locals, request }) => {
	...
	// Lire le body pour obtenir le nombre de millisecondes à ajouter
	...
	// Ajouter le temps à la durée du timer
	const updatedTimer = await prisma.runTimer.update({
		where: { id: currentRun.timer.id },
		data: {
			durationMs: currentRun.timer.durationMs + ms
		}
	});
