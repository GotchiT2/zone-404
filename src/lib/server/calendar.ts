...
export async function getAvailableSlots(calendarKey: string): Promise<Slot[]> {
  ...
  return events
    .filter(
      (event) =>
        !!event.id && !!event.summary && event.summary.startsWith("[LIBRE]"),
    )
    .map((event) => ({
...
export async function reserveSlot(
  ...
  return {
    ...
    start: savedEvent.start?.dateTime || savedEvent.start?.date || "",
    end: savedEvent.end?.dateTime || savedEvent.end?.date || "",
  };
}
...
export async function holdSlot(
  ...
  if (!summary.startsWith("[LIBRE]")) {
    const err = new Error(
      "Ce créneau n’est pas libre (déjà réservé ou en cours)",
    );
...
export async function startReservation(
  ...
  if (players < 2 || players > 6) {
    const err = new Error(
      "Le nombre de joueurs doit être compris entre 2 et 6",
    );
    (err as any).status = 400;
    throw err;
  }
  ...
  // On ne peut lancer une resa que depuis un créneau [LIBRE]
  if (!summary.startsWith("[LIBRE]")) {
    const err = new Error(
      "Ce créneau n’est pas libre (déjà en cours ou réservé)",
    );
    (err as any).status = 409;
    throw err;
  }
  ...
  const now = Date.now();
  const expirationDate = new Date(now + 10 * 60 * 1000).toISOString();
  ...
  // On stocke des infos structurées dans extendedProperties.private
